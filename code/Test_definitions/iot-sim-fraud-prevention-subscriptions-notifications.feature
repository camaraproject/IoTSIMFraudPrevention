Feature: CAMARA IoT SIM Fraud Prevention Subscriptions, v0.2.0-rc.1 - Notification Delivery
  ...
  Background: Common setup
    ...
    And the resource "/iot-sim-fraud-prevention-subscriptions/v0.2rc1/subscriptions"
  ...
  Scenario: Receive notification when IMEI changes
    ...
    And the subscription type is "org.camaraproject.iot-sim-fraud-prevention.v0.imei-change"
    ...
    And the notification payload complies with the schema "#/components/schemas/NotificationEvent"
  ...
  Scenario: Receive notification when area changes
    Given a subscription for device with phoneNumber "+123456789"
    And the subscription type is "org.camaraproject.iot-sim-fraud-prevention.v0.area-change"
    And the sink is set to "https://mock-sink.example.com/notify"
    And the sink uses an access token "valid-bearer-token"
